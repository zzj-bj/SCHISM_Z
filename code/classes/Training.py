...
class Training():
    ...
    def __init__(self, **kwargs):
        ...
        if metrics_str:
            ...
            self.metrics = ["MeanIoU"] # For multiclass problems
    ...
    def initialize_model(self) -> nn.Module:
        ...
        return model_class(**typed_required_params, **optional_params)
    ...
    def get_metrics(self, metrics_list):
           ...
           metrics_mapping = {
               "GeneralizedDiceScore": GeneralizedDiceScore(num_classes=num_classes),
               "HausdorffDistance": HausdorffDistance(num_classes=num_classes,  
                                                      include_background=True,
                                                      distance_metric="euclidean",
                                                      directed=False,
                                                      input_format="index"),
               "MeanIoU": MeanIoU(num_classes=num_classes),
               "DiceScore" : DiceScore(num_classes=num_classes),
           }
    ...
    def get_losses(self):
        ...
        is_binary = self.model.num_classes == 1
                ...
                with tqdm(total=len(self.dataloaders[phase]), unit="batch") as pbar:
                    for inputs, labels, _, _ in self.dataloaders[phase]:
                        ...
                        with torch.no_grad():
                            '''
                            #preds = (
                            #    (outputs > 0.5).int() if self.model.num_classes == 1 else torch.argmax(outputs, dim=1)
                            #)
                            if self.model.num_classes == 1:
                                preds = (outputs > 0.5).int()  # Binary case, should output [batch_size, height, width]
                            else:
                                preds = torch.argmax(outputs, dim=1)  # Multiclass case, outputs [batch_size, height, width]
                            ...
                            # Mask out background (-1) from labels and predictions
                            labels = torch.where(labels == -1, torch.tensor(0, dtype=labels.dtype).to(self.device), labels)
                            mask = labels != -1  # Create a mask for valid (non-background) pixels
                            preds = (outputs > 0.5).int() if self.model.num_classes == 1 else torch.argmax(outputs, dim=1)
    
                            # Apply the mask to exclude background class
                            labels = labels[mask]
                            preds = preds[mask]
    
    
    
                            # Check if preds and labels are now correctly shaped
                            print(f"Preds shape: {preds.shape}")
                            print(f"Labels shape: {labels.shape}")
                            
                            # Calculate metrics for valid pixels only
                            for metric_name, metric_fn in zip(self.metrics, metrics):
                                running_metrics[metric_name] += metric_fn(preds, labels).item()
