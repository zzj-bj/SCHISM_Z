...
import keras
from tensorflow.keras.layers import GlobalAveragePooling2D, AveragePooling2D, Flatten, Dense, UpSampling2D, add, Dropout, Conv2D, Conv2DTranspose, MaxPooling2D, concatenate, BatchNormalization, Activation, Reshape
...
from tensorflow.keras.metrics import BinaryIoU, BinaryAccuracy, BinaryCrossentropy, CategoricalAccuracy, CategoricalCrossentropy, OneHotMeanIoU, OneHotIoU
import os
import numpy as np
...
class ResUNet:
    ...
    def __init__(self, **kwargs):
        ...
        if "data" in kwargs:
            if isinstance(kwargs['data'],(Util)):
                ...
                self.FILE_TXT = self.FILE_TXT + "\nval_split = " + str(self.util.getValidationSplit())
                ...
                for metric_name in self.metrics:
                    ...
                    if metric_name == 'OneHotIoU' or metric_name == 'OneHotMeanIoU':
                        ...
                        if metric_name == 'OneHotIoU':
                            ...
                            metric_instance = globals()[metric_name](num_classes=self.numClass, target_class_ids=target_class_ids)
                        else:  
                            metric_instance = globals()[metric_name](num_classes=self.numClass)
                    else:
    ...
    def dataGenerator(self, seed, batch_size, isTrainSet, forVisualisation=False):
        if isTrainSet:
            ...
            genX1 = image_datagen.flow(self.Xtrain, self.Ytrain, batch_size=batch_size, seed=seed, shuffle=isTrainSet)
            image_datagen.fit(self.Ytrain)
            genX2 = image_datagen.flow(self.Ytrain, self.Xtrain, batch_size=batch_size, seed=seed, shuffle=isTrainSet)
        else:
    ...
    def setModel(self):
        ...
        if self.pretrained:
            ...
            if self.numClass > 2:
                self.model = sm.Unet(self.backbone, encoder_weights='imagenet', classes=self.numClass, activation='softmax')
            ...
            inputs = keras.layers.Input(shape=(self.img_height,self.img_width, self.Xtrain.shape[-1]))
            #Architecture borrowed from https://github.com/AhadMomin/semantic-segmentation-digital-rock-physics 
            conv1 = Conv2D(self.featuremaps * 1, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(inputs)
            conv1 = batch_Norm_Activation(conv1)
            conv1 = Conv2D(self.featuremaps * 1, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv1)
            conv1 = batch_Norm_Activation(conv1)
            pool1 = MaxPooling2D((2, 2))(conv1)
    
            conv2 =Conv2D(self.featuremaps * 2, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(pool1)
            conv22 = batch_Norm_Activation(conv2)
            shortcut=Conv2D(self.featuremaps * 2, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv22)
            shortcut= batch_Norm_Activation(shortcut)
            conv2 =Conv2D(self.featuremaps * 2, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv22)
            conv2 = batch_Norm_Activation(conv2)
            conv2 =Conv2D(self.featuremaps * 2, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv2)
            conv2 = batch_Norm_Activation(conv2)
            conv2 = add([shortcut, conv2])
            pool2 =MaxPooling2D((2, 2))(conv2)
    
            conv3 =Conv2D(self.featuremaps * 4, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(pool2)
            conv33 = batch_Norm_Activation(conv3)
            shortcut1=Conv2D(self.featuremaps * 4, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv33)
            shortcut1= batch_Norm_Activation(shortcut1)
            conv3 =Conv2D(self.featuremaps * 4, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv33)
            conv3 = batch_Norm_Activation(conv3)
            conv3 =Conv2D(self.featuremaps * 4, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv3)
            conv3 = batch_Norm_Activation(conv3)
            conv3 = add([shortcut1, conv3])
            pool3 =MaxPooling2D((2, 2))(conv3)
            
            conv4 = Conv2D(self.featuremaps * 8, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(pool3)
            conv44 = batch_Norm_Activation(conv4)
            shortcut2= Conv2D(self.featuremaps * 8, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv44)
            shortcut2= batch_Norm_Activation(shortcut2)
            conv4 = Conv2D(self.featuremaps * 8, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv44)
            conv4 = batch_Norm_Activation(conv4)
            conv4 =Conv2D(self.featuremaps * 8, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(conv4)
            conv4 = batch_Norm_Activation(conv4)
            conv4 = add([shortcut2, conv4])
            pool4 = MaxPooling2D((2, 2))(conv4)
    
            convm = Conv2D(self.featuremaps * 16, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(pool4)
            convm = batch_Norm_Activation(convm)
            shortcut3= Conv2D(self.featuremaps * 16, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(convm)
            shortcut3= batch_Norm_Activation(shortcut3)
            convm =Conv2D(self.featuremaps * 16, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(convm)
            convm = batch_Norm_Activation(convm)
            convm = Conv2D(self.featuremaps * 16, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(convm)
            convm = batch_Norm_Activation(convm)
            convm = add([shortcut3, convm])
    
            deconv4 =Conv2DTranspose(self.featuremaps * 8, (2, 2), strides=(2, 2), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(convm)
            uconv4 = concatenate([deconv4, conv4])
            uconv4 =Conv2D(self.featuremaps * 8, (2, 2), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv4)
            uconv4 = batch_Norm_Activation(uconv4)
            shortcut4=Conv2D(self.featuremaps * 8, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv4)
            ...
            deconv3 =Conv2DTranspose(self.featuremaps * 4, (2, 2), strides=(2, 2), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv4)
            uconv3 = concatenate([deconv3, conv3])
            uconv3 =Conv2D(self.featuremaps * 4, (2, 2), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv3)
            uconv3 = batch_Norm_Activation(uconv3)
            shortcut5=Conv2D(self.featuremaps * 4, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv3)
            ...
            deconv2 =Conv2DTranspose(self.featuremaps * 2, (2, 2), strides=(2, 2), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv3)
            uconv2 = concatenate([deconv2, conv2])
            uconv2 =Conv2D(self.featuremaps * 2, (2, 2), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv2)
            uconv2 = batch_Norm_Activation(uconv2)
            shortcut6=Conv2D(self.featuremaps * 2, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv2)
            ...
            deconv1 =Conv2DTranspose(self.featuremaps * 1, (2, 2), strides=(2, 2), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv2)
            uconv1 = concatenate([deconv1, conv1])
            uconv1 =Conv2D(self.featuremaps * 1, (2, 2), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv1)
            uconv1 = batch_Norm_Activation(uconv1)
            shortcut7=Conv2D(self.featuremaps * 1, (3, 3), padding=self.padding, kernel_regularizer=regularizers.L2(l2=self.L2), kernel_constraint=keras.constraints.max_norm(self.maxNorm))(uconv1)
            ...
            if self.numClass > 2:
               final = Conv2D(self.numClass, (1,1), padding=self.padding, activation='softmax')(uconv1)
            elif self.numClass <= 2:
    ...
    def run(self, forVisualisation=False):
        ...
        if len(self.early_stopping) > 0:
            for early_stopping in self.early_stopping:
                early_stopping_tmp = keras.callbacks.EarlyStopping(monitor=early_stopping, mode='max', verbose=1, patience=self.patience, restore_best_weights=True)
                callbacks.append(early_stopping_tmp)
        if self.loss_early_stopping:
            early_stopping_loss = keras.callbacks.EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=self.patience, restore_best_weights=True)
        ...
        if len(self.num_gpus) > 0:
            steps_per_epoch=self.Xtrain.shape[0]//(self.batch_size * len(self.num_gpus))
            validation_steps=self.Xtest.shape[0]//(self.batch_size * len(self.num_gpus))
        ...
        if self.dataGeneration:
            ...
            train_generator = self.dataGenerator(seed=105, batch_size=self.batch_size, isTrainSet=True, forVisualisation=False)
            test_generator = self.dataGenerator(seed=105, batch_size=batch_size_val, isTrainSet=False, forVisualisation=False)
            ...
            if self.save_model:
                ...
                with open(os.path.join(newpath,'weights.json'), "w") as json_file:
                    ...
                    self.model.save_weights(os.path.join(newpath,'weights.h5'))
